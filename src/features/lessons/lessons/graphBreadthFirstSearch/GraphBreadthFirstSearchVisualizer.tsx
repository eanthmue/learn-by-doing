...
function parseLevelMap(value: string | number | boolean | null | undefined): Map<number, number> {
  ...
  body.split(",").forEach((entry) => {
    const [nodeText, levelText] = entry.split(":").map((part) => part.trim());
    const node = Number(nodeText);
    const level = Number(levelText);
    if (Number.isFinite(node) && Number.isFinite(level)) {
...
export function GraphBreadthFirstSearchVisualizer({
  values,
  steps,
  stepIndex,
  onStepIndexChange,
  ...
  const queue = parseNumberList(current.variables?.queue);
  const levels = parseLevelMap(current.variables?.levels);
  const activeEdge = parseEdge(current.variables?.edge);
  const activeNode = current.activeIndex;

  return (
    <div className="visualizer-panel graph-viz-panel bfs-viz-panel" aria-label="Graph Breadth-First Search (BFS) visualizer">
      ...
      <div className="graph-viz-diagram" aria-label="Breadth-First Search (BFS) graph diagram">
        <svg
        ...
        >
          ...
          {positions.map((pos, node) => {
            ...
            const isQueued = queue.includes(node);
            return (
              <g key={`node-${node}`}>
                <circle
                  cx={pos.x}
                  cy={pos.y}
                  r={18}
                  className={`graph-viz-node${isVisited ? " touched" : ""}${isActive ? " active" : ""}${isQueued ? " bfs-queued" : ""}`}
                />
                <text
                  x={pos.x}
                  y={pos.y}
      ...
      <div className="dfs-state-grid bfs-state-grid" aria-label="BFS state">
        ...
        <div className="graph-viz-repr dfs-state-card">
          <span className="graph-viz-repr-title">Queue</span>
          <div className="dfs-pill-row">
            {queue.length > 0 ? queue.map((node, index) => (
              <span className="dfs-pill queue" key={`queue-${node}-${index}`}>
                {node}
              </span>
            )) : <span className="dfs-empty">empty</span>}
          </div>
      ...
      <div className="graph-viz-repr" aria-label="BFS levels">
        <span className="graph-viz-repr-title">Levels from Start</span>
        <div className="bfs-level-list">
          {Array.from({ length: numNodes }, (_, node) => (
            <span
              className={`bfs-level-token${visited.has(node) ? " visited" : ""}${activeNode === node ? " active" : ""}`}
              key={`level-${node}`}
            >
              <span>{node}</span>
              <strong>{levels.has(node) ? levels.get(node) : "-"}</strong>
            </span>
          ))}
        </div>
      </div>
